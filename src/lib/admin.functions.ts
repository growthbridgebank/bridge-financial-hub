...
async function profileMap(
  ...
  if (!unique.length) {
    return {} as Record<string, { name: string; email: string }>;
  }
  ...
  .handler(async ({ context }) => {
    ...
    const [
      ...
      count("kyc_submissions", (q) =>
        q.in("status", ["pending", "in_review", "unverified"]),
      ),

      count("accounts"),

      count("transactions", (q) =>
        q.eq("direction", "credit").in("status", ["pending", "processing"]),
      ),

      count("transactions", (q) =>
        q.eq("direction", "debit").in("status", ["pending", "processing"]),
      ),

      count("transfers", (q) =>
        q.in("status", ["pending", "under_review", "processing"]),
      ),
...
export const listKyc = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.status === "pending") {
      q = q.in("status", ["unverified", "pending", "in_review"]);
  ...
  .inputValidator((i) =>
    z
      .object({
        ...
        status: z.enum(["pending", "active", "frozen", "closed"]),
        reason: z.string().trim().max(400).optional().or(z.literal("")),
      })
      .parse(i),
  )
...
export const listTransfers = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    return (rows ?? []).map((r: any) => ({
      ...r,
      customer: names[r.sender_user_id]?.name ?? "—",
      customerEmail: names[r.sender_user_id]?.email ?? "",
    }));
  });
